...
int main(int argc, char ** argv)
{
  ...
  const unsigned K = atoi(argv[argument++]);
  ...
  const char * summaryfile = argv[argument++];
  const char * outfile_basename = argv[argument++];
  const unsigned seed = atoi(argv[argument++]);

  //figure out bounds on size of rest of chromosome
  double CSIZE = 1./(1.-(0.5-2*littler)/0.5);
  const double LEFT = -CSIZE/2.,RIGHT=CSIZE/2.;
  const double LITTLER=0.25; //r per "gamete"
  ...
  */
  ...
  //add in the remaining k selected sites
  vector<double> coeff;
  for(unsigned i=0;i<maxgams-1;++i)
    {
      coeff.push_back(1./double(i+1));
    }

  transform(coeff.begin(),coeff.end(),coeff.begin(),bind2nd(divides<double>(),accumulate(coeff.begin(),coeff.end(),0.)));

  vector<double> mcoeff(maxgams,1./double(maxgams));
  boost::function< size_t (size_t) > rand = boost::bind(&gsl_ran_flat, r, 0,double(maxgams));
  ...
  for( unsigned i = 0 ; i < K ; ++i )
    {
      ...
      double sum = 0.;
      unsigned freq=0;
      for( unsigned j=0;freq==0&&j<coeff.size(); ++j )
	{
	  sum+=coeff[j];
	  if( x <= sum ) 
	    {
      ...
      vector<unsigned> hasit(freq,1);
      for(unsigned i = 0 ; i < maxgams-freq ; ++i)
	{
	  hasit.push_back(0);
	}
      std::random_shuffle(hasit.begin(),hasit.end(),rand);      
      double pos = (gsl_rng_uniform(r) < 0.5) ? gsl_ran_flat(r,LEFT,0) : gsl_ran_flat(r,1,RIGHT);
      list<mtype>::iterator ssite_itr_k = imutations.insert(imutations.end(), breakpoint( pos, freq,
											  numeric_limits<unsigned>::max(),s,h,false ) );
      for( unsigned j = 0 ; j < maxgams ; ++j )
	{
	  if( hasit[j] )
	    {
	      igametes[j].smutations.push_back(ssite_itr_k);
	    }
	}
    }
  
  //get dist of fitness in base population, to address the silliness in the Schlotterer group paper
  ostringstream summary_buffer;
  for(unsigned i = 0 ; i < igametes.size() ; ++i )
    {
      for(unsigned j=i ; j < igametes.size() ; ++j)
	{
	  unsigned ni = igametes[i].n, nj = igametes[j].n;
	  double pmating = (i==j) ? pow(double(ni)/double(maxgams),2.) : 2.*(double(ni)/double(maxgams))*(double(nj)/double(maxgams));
	  double w = multiplicative_diploid()( igametes.begin()+i,igametes.begin()+j );
	  summary_buffer <<"w "<< w << '\t' << pmating  << '\n';
	}
    }
  for( list<mtype>::const_iterator i = imutations.begin(); i != imutations.end() ; ++i )
    {
      if( ! i->neutral )
	{
	  summary_buffer << "m " << i->pos << '\t' << i->n << '\n';
	}
    }
  
  filtering_ostream sumout;
  sumout.push(gzip_compressor());
  sumout.push(file_sink(summaryfile),ios_base::binary|ios_base::out);
  sumout << summary_buffer.str();
  sumout.pop();
  sumout.pop();
      ...
      if( ngens[LENGTH] )
	{
	  for( unsigned rep=0;rep<nreps_sel+nreps_control;++rep )
	      ...
	      for(;gen[rep]<ngens[LENGTH];++gen[rep])
		{
		  ...
		  unsigned nrec = recombine_breakpoints(r, &vgametes[rep], &vmutations[rep],2*N, LITTLER, boost::bind(gsl_ran_flat,r,LEFT,RIGHT));
		  assert(check_sum( vgametes[rep],2*N ) );
		  assert( mutations_are_sorted( vgametes[rep] ) );
		  assert( nmuts_not_empty( vgametes[rep] ) );
		  remove_lost(&vmutations[rep],gen[rep],2*N);
